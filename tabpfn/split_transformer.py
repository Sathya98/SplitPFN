...
class SplitTransformerModel(nn.Module):
    def __init__(self, encoder, n_out, ninp, nhead, nhid, nlayers, dropout=0.0, style_encoder=None, y_encoder=None,
        ...
        self.transformer_encoder = nn.TransformerEncoder(nn.TransformerEncoderLayer(ninp, nhead, nhid, dropout, activation=activation), nlayers)
        self.decoder = TransformerDecoder(TransformerDecoderLayer(ninp, nhead, nhid, dropout), nlayers)
        self.clf_head = nn.Sequential(nn.Linear(ninp, nhid), nn.ReLU(), nn.Linear(nhid, n_out)) if decoder is None else decoder
        self.ninp = ninp
...
class TransformerDecoderLayer(nn.Module):
    def _init_(self, d_model, nhead, dim_feedforward=2048, dropout=0.1):
        super()._init_()
        ...
        # Layer normalization (omitting the first one as per your request)
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None, tgt_key_padding_mask=None, memory_key_padding_mask=None):
        # Self attention (removed masking and first norm)
        tgt2 = self.self_attn(tgt, tgt, tgt, attn_mask=tgt_mask,
                              key_padding_mask=tgt_key_padding_mask)[0]
        tgt = tgt + self.dropout1(tgt2)

        # Add & norm (omitting the first norm)
        tgt2 = self.norm2(tgt)
