...
class SplitTransformerModel(nn.Module):
    def __init__(
        ...
        self.decoder = TransformerDecoder(
            TransformerDecoderLayer(ninp, nhead, nhid, dropout), nlayers
        )
        self.clf_head = nn.Sequential(
            nn.Linear(ninp, nhid), nn.ReLU(), nn.Linear(nhid, n_out)
        )
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1):
        super().__init__()
        # Multi-head attention layers (removed the first self attention)
        ...
        # Layer normalization (omitted the first one as per your request)
        self.norm1 = nn.LayerNorm(
            d_model
        )  # Renamed to norm1 since it's now the first norm in the sequence
